...
#満腹度
#execute if entity @a[nbt=!{ActiveEffects:[{Id:23}]},limit=1] as @s[nbt=!{ActiveEffects:[{Id:23}]}] run effect give @s minecraft:saturation 1000000 0 true
execute if entity @s[tag=!not-blind-now,nbt={ActiveEffects:[{Id:15}]}] run function ex3:general/effects/blindness/check
...
#耐性
execute if entity @s[nbt=!{ActiveEffects:[{Id:11}]}] run effect give @s minecraft:resistance 1000000 4 true
