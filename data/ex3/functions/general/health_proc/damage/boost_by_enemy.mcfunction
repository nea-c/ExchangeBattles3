...
#ギャンブラー
execute at @s if entity @a[tag=!hurt,tag=hurt_by,distance=..7,advancements={ex3:general/attack/melee_dealt=true},scores={support=11},sort=nearest,limit=1] run tag @s add support11-1
...
#フォースガーナー
execute at @s if entity @a[tag=!hurt,tag=hurt_by,distance=..7,advancements={ex3:general/attack/melee_dealt=true},tag=skill4-now,limit=1] run scoreboard players set #boost value 2
execute at @s if entity @a[tag=!hurt,tag=hurt_by,distance=..7,advancements={ex3:general/attack/melee_dealt=true},tag=skill4-now,limit=1] run scoreboard players operation @s damage_resist *= #boost value
